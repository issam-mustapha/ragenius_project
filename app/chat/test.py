...
# -------------------------------
# 4️⃣ Fonctions CRUD profil
# -------------------------------
def load_profile(store, user_id: str) -> UserProfile:
    docs = store.search(("users", user_id, "profile"))
    if docs:
        try:
            return UserProfile(**docs[0].value)
        except ValidationError as e:
            logging.warning(f"Profile in DB invalid, resetting: {e}")
    # Retourne un profil vide valide
    return UserProfile()


def save_profile(store, user_id: str, profile: UserProfile):
    # Sérialisation JSON sûre avec gestion des datetime
    profile_json = profile.model_dump_json()  # renvoie une string JSON
    store.put(("users", user_id, "profile"), "main", json.loads(profile_json))


# -------------------------------
# 5️⃣ Extraction et validation JSON LLM
# -------------------------------
def extract_and_validate_json(llm_text: str) -> Optional[UserProfile]:
    """
    Extrait un JSON depuis le texte renvoyé par le LLM et valide avec UserProfile.
    """
    try:
...
def merge_profiles(existing: UserProfile, new: UserProfile) -> UserProfile:
    ...
    for field in existing.model_fields:
        ...
        if isinstance(old_value, list) and isinstance(new_value, list):
            ...
            setattr(updated, field, combined)
        elif isinstance(old_value, dict) and isinstance(new_value, dict):
            merged_dict = {**old_value, **new_value}
...
"""
    ...
    if updated_profile:
        ...
        logging.warning("Profile update skipped: no valid JSON returned")

    return state

# -------------------------------
# 8️⃣ Node : réponse personnalisée
# -------------------------------
...
print("🧪 Chat test (type 'exit' pour quitter)\n")
...
while True:
    ...
    for chunk in graph.stream(message, config, stream_mode="values"):
